...
int main(int argc, char** argv) {
  ...
  // Wait for a point cloud message
  sensor_msgs::PointCloud2ConstPtr cloud =
      ros::topic::waitForMessage<sensor_msgs::PointCloud2>(
          "head_camera/depth_registered/points");
  
  if (!cloud) {
    std::cerr << "Error: No point cloud received." << std::endl;
    return 1;
  }
  
  // Set up transform listener to convert cloud to base_link frame
  ...
  // Save the transformed point cloud to a bag file
